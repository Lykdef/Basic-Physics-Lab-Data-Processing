...
const props=defineProps<{project:Project;fits:Record<string,FitState>}>();
...
const invalid=computed(()=>config.value.variables.some(v=>v.source==='manual' && (invalidInputs.value[v.id+':value'] || invalidInputs.value[v.id+':uncertainty'])));
const sources=computed(()=>sourceOptions(props.project));
const inputs=computed(()=>{try{return resolveInputs(config.value,props.project,props.fits);}catch{return null;}});
...
watch(()=>JSON.stringify([config.value,props.project.datasets,props.fits,invalid.value]),()=>{
  const version=++generation;clearTimeout(timer);result.value=null;error.value='';pending.value=true;
  timer=setTimeout(async()=>{try{if(invalid.value)throw new Error('请输入有效数值，标准不确定度须为非负数');const r=await calculate<PropagationResult>(propagationRequest(config.value,props.project,props.fits));if(version===generation)result.value=r;}catch(e){if(version===generation)error.value=(e as Error).message;}finally{if(version===generation)pending.value=false;}},400);
...
<template>
  <section class="advanced-panel propagation-panel">
    ...
    <div class="advanced-table-wrap"><table class="advanced-table"><thead><tr><th>变量</th><th>来源</th><th>最佳估计值</th><th>标准不确定度</th><th></th></tr></thead><tbody><tr v-for="(v,i) in config.variables" :key="v.id"><td><input :aria-label="`变量 ${i+1} 符号`" :value="v.symbol" @change="variable(i,'symbol',$event)"/></td><td><select :aria-label="`变量 ${i+1} 来源`" :value="v.source" @change="variable(i,'source',$event)"><option value="manual">手动输入</option><option v-if="v.source!=='manual' && !sources.some(s=>s.id===v.source)" :value="v.source">来源已失效</option><option v-for="s in sources" :key="s.id" :value="s.id">{{s.label}}</option></select></td><td><DecimalInput v-if="v.source==='manual'" :aria-label="`变量 ${i+1} 估计值`" :model-value="v.value" @update:model-value="update(c=>c.variables[i]!.value=$event)" @invalid="invalidInputs[v.id+':value']=$event"/><output v-else>{{inputs?.[i]?.value ?? '—'}}</output></td><td><DecimalInput v-if="v.source==='manual'" nonnegative :aria-label="`变量 ${i+1} 标准不确定度`" :model-value="v.uncertainty" @update:model-value="update(c=>c.variables[i]!.uncertainty=$event)" @invalid="invalidInputs[v.id+':uncertainty']=$event"/><output v-else>{{inputs?.[i]?.uncertainty ?? '—'}}</output></td><td><button :aria-label="`删除输入变量 ${i+1}`" :disabled="config.variables.length<=1" @click="remove(v.id)">删除</button></td></tr></tbody></table></div>
    <button :disabled="config.variables.length>=16" @click="add">＋ 添加输入变量</button>
    <details v-if="pairs.length" class="correlations"><summary>相关系数</summary><p class="calculation-note">未设置的输入按独立量处理；同一拟合的参数自动保留相关性。</p><div class="advanced-fields"><label v-for="pair in pairs" :key="pair.key">{{pair.a.symbol}} ↔ {{pair.b.symbol}}<input type="number" min="-1" max="1" step="0.01" :aria-label="`相关系数 ${pair.a.symbol} ${pair.b.symbol}`" :disabled="pair.auto!==null" :value="pair.auto ?? config.correlations[pair.key] ?? 0" @input="Number.isFinite(Number(($event.target as HTMLInputElement).value)) && update(c=>c.correlations[pair.key]=Math.max(-1,Math.min(1,Number(($event.target as HTMLInputElement).value))))"/></label></div></details>
    <div class="advanced-fields"><label>不确定度有效数字<select aria-label="传播不确定度有效数字" :value="config.digits" @change="update(c=>c.digits=Number(($event.target as HTMLSelectElement).value) as 1|2)"><option :value="1">1 位</option><option :value="2">2 位</option></select></label><label>包含因子 k<input type="number" min="0.01" step="any" :value="config.k" aria-label="包含因子" @input="Number(($event.target as HTMLInputElement).value)>0 && update(c=>c.k=Number(($event.target as HTMLInputElement).value))"/></label></div>
    <p v-if="pending" role="status">计算中…</p><p v-if="error" class="calculation-error" role="alert">{{error}}</p>
    <template v-if="result"><div class="propagation-result"><span>y ± u</span><output aria-label="传播最佳估计值">{{formatEstimate(result.value,result.uncertainty,config.unit,config.digits)}}</output><span>相对标准不确定度：{{result.relative===null?'—':(result.relative*100).toPrecision(6)+'%'}}</span><span>扩展不确定度 U（k = {{config.k}}）：{{formatUncertainty(result.expanded,config.digits)}} {{config.unit}}</span></div>
