...
import {derivedDataset,axisInputs} from './derived';
import {mathParts} from './math-label';
import MathLabel from './MathLabel.vue';
import { exportPlotPng } from './plot-export';
...
watch(()=>dataset.value.kind,kind=>{if(kind==='combined')tab.value='uncertainty';else if(tab.value==='uncertainty' || (kind==='paired' && tab.value==='statistics'))tab.value='data';},{flush:'sync',immediate:true});
const transformed=computed(()=>derivedDataset(dataset.value,selectedColumn.value));
...
const exportingPlot = ref(false);
const exportOptions = ref({title:'',table:false,formula:false,r2:false});
async function exportImage() {
  if (!plotSvg.value || exportingPlot.value) return;
  exportingPlot.value = true;
  try { await exportPlotPng(plotSvg.value, dataset.value.name, plot.value.ylabel, {...exportOptions.value,dataset:transformed.value.dataset,fit:dataset.value.kind==='paired' && !fits[dataset.value.id]?.pending ? fits[dataset.value.id]?.result ?? undefined : undefined}); }
  catch (error) { notice(error instanceof Error ? error.message : '图像导出失败，请重试'); }
  finally { exportingPlot.value = false; }
}
const recovery = ref('');
try {
...
const modal = ref(''); const form = ref({name:'',symbol:'',display_symbol:'',unit:'',precision:2,kind:'repeated',reason:''}); const modalError = ref(''); const targetRow = ref(''); const targetDatasetId=ref('');
function deleteDataset(value=dataset.value.id){targetDatasetId.value=value;openModal('delete-dataset');}
function openModal(name: string) { modal.value = name; modalError.value = ''; form.value = {name:'',symbol:'',display_symbol:'',unit:'',precision:2,kind:'repeated',reason:''}; if (name === 'column') form.value = {...form.value,...column.value}; if (name === 'rename') form.value.name = dataset.value.name; }
function submitModal() {
  ...
  if (['column','add-column'].includes(modal.value)) {
    ...
    if (dataset.value.derived?.some(c=>c.symbol===f.symbol) || dataset.value.columns.some((c,i) => c.symbol === f.symbol && (modal.value === 'add-column' || i !== selectedColumn.value))) { modalError.value = '变量符号已存在'; return; }
    if (!Number.isInteger(Number(f.precision)) || f.precision < 0 || f.precision > 8) { modalError.value = '显示精度须为 0–8 位'; return; }
    const c: Column = { id: modal.value === 'column' ? column.value.id : id(), name:f.name.trim(),symbol:f.symbol,display_symbol:f.display_symbol.trim(),unit:f.unit,precision:Number(f.precision) };
...
const axisFields=computed(()=>axisInputs(dataset.value,selectedColumn.value));
function updateAxis(axis:'x'|'y',key:'expression'|'unit',event:Event){const value=(event.target as HTMLInputElement).value;edit(()=>{dataset.value.axis_inputs={x:{...axisFields.value.x},y:{...axisFields.value.y}};dataset.value.axis_inputs[axis][key]=value;delete dataset.value.derived;});}
        ...
        <template v-if="workspace==='workbench'">
          ...
          <div class="work-area" :class="{'full-width':tab!=='data'}">
            <section class="data-panel panel">
              ...
              <div class="view-toolbar"><div class="view-tabs"><button v-if="dataset.kind!=='combined'" :class="{active:tab==='data'}" @click="tab='data'"><Table2 :size="15"/>数据表格</button><button v-if="dataset.kind==='repeated'" :class="{active:tab==='statistics'}" @click="tab='statistics'"><Activity :size="15"/>统计评定</button><button v-if="dataset.kind!=='combined'" :class="{active:tab==='plot'}" @click="tab='plot'"><ChartNoAxesCombined :size="15"/>数据预览</button><button v-if="dataset.kind!=='combined'" :class="{active:tab==='propagation'}" @click="tab='propagation'">科学计算</button><button v-if="dataset.kind==='combined'" class="active" @click="tab='uncertainty'">科学计算</button></div><div class="table-tools"><span v-if="dataset.simulated" class="simulation">模拟数据</span><button :title="project.datasets.length > 1 ? '删除数据组' : '至少保留一个数据组'" aria-label="删除数据组" :disabled="project.datasets.length <= 1" @click="deleteDataset()"><Trash2 :size="14"/></button><button title="重命名数据组" @click="openModal('rename')"><Pencil :size="14"/></button><button title="撤销" :disabled="!undoStack.length" @click="undo"><Undo2 :size="16"/></button><button title="重做" :disabled="!redoStack.length" @click="redo"><Redo2 :size="16"/></button><span/><button v-if="dataset.kind!=='combined'" title="显示或隐藏已排除数据" :class="{filtered:!showExcluded}" @click="showExcluded=!showExcluded"><ListFilter :size="16"/></button><button v-if="dataset.kind!=='combined'" title="添加变量列" @click="openModal('add-column')"><Plus :size="17"/></button></div></div>
