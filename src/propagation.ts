...
export function sourceOptions(project:Project){return project.datasets.flatMap(d=>[
  ...
  ...(d.kind==='paired' && d.fit?.enabled ? d.fit.parameters.map((_,i)=>({id:`fit:${d.id}:${i}`,label:`${d.name} · 拟合参数 ${d.fit!.model==='polynomial'?'a'+i:['a','b','c'][i]}`})):[])
]);}
export function resolveInputs(config:PropagationConfig,project:Project,fits:Record<string,FitState>,valuesOnly=false){
  return config.variables.map(v=>{
    if(v.source==='manual')return {...v,uncertainty:valuesOnly?0:v.uncertainty};
    ...
    if(kind==='fit'){
      ...
      if(!state?.result || state.result.parameters[i]===undefined)throw new Error(`${v.symbol} 的拟合参数不可用`);
      return {...v,value:state.result.parameters[i]!,uncertainty:0};
    }
    ...
    if(stats.mean===null || (!valuesOnly && stats.combined===null))throw new Error(`${v.symbol} 需要至少两次有效测量及已设置的 B 类不确定度`);
    return {...v,value:stats.mean,uncertainty:valuesOnly?0:stats.combined!};
  });
}
...
export function autoCorrelation(a:PropagationConfig['variables'][number],b:PropagationConfig['variables'][number],fits:Record<string,FitState>):number|null{
  ...
  if(ak==='fit' || bk==='fit')return 0;
  return null;
}
export function propagationRequest(config:PropagationConfig,project:Project,fits:Record<string,FitState>,valuesOnly=false){
  const variables=resolveInputs(config,project,fits,valuesOnly);
  ...
  return {operation:'propagate',values_only:valuesOnly,expression:config.expression,variables,correlation,k:config.k};
}
