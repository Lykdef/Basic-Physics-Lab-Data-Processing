...
test('按数据类型显示页面，切组回到可用页面',async({page})=>{
 await page.goto('/');
 await expect(page.locator('.view-tabs button')).toHaveText(['数据表格','统计评定','数据预览','科学计算']);
 await page.getByRole('button',{name:'科学计算',exact:true}).click();
 await page.locator('.dataset-tabs').getByRole('button',{name:'伏安法测电阻'}).click();
 await expect(page.locator('.view-tabs button')).toHaveText(['数据表格','数据预览','科学计算']);
 await expect(page.locator('.propagation-panel')).toBeVisible();
 ...
 await expect(page.getByLabel('自变量 · 横轴')).toBeDisabled();
 await expect(page.getByRole('checkbox',{name:'曲线拟合',exact:true})).toHaveCount(0);
 await page.getByRole('button',{name:'数据表格',exact:true}).click();
 await page.locator('.inspector select').first().selectOption('paired');
 await expect(page.getByRole('button',{name:'科学计算',exact:true})).toBeVisible();
...
test('逐字符录入 0.006、小数及科学计数法不被数值回填截断',async({page})=>{
 ...
 await u.fill('0.006');await page.reload();await page.getByRole('button',{name:'科学计算',exact:true}).click();await expect(u).toHaveValue('0.006');
});
