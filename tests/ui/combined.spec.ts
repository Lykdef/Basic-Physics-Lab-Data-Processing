...
test('合成数据组仅不确定度页面，各组独立保存并可重新导入',async({page})=>{
 ...
 await add('合成一');await expect(page.locator('.view-tabs button')).toHaveText(['不确定度']);await expect(page.locator('.measurement-grid')).toHaveCount(0);await expect(page.locator('.inspector')).toHaveCount(0);
 ...
 await page.reload();await expect(page.locator('.view-tabs button')).toHaveText(['不确定度']);await expect(page.getByLabel('传播最佳估计值')).toHaveText('(2.00 ± 0.05)');
